 ...
 * @author RSIC-Zero Team
 * @notice Extended interface for confidential fungible tokens with additional methods
 * @dev This interface extends the standard IConfidentialFungibleToken with local-specific methods
 */
interface ILocalConfidentialFungibleToken is IConfidentialFungibleToken {
    /**
     * @notice Transfer encrypted tokens from one account to another
    ...
    /**
     * @notice Transfer encrypted tokens to another account
     * @param recipient The address to transfer to
     * @param amount The encrypted amount to transfer
     * @return The encrypted amount actually transferred
     */
    ...
    /**
     * @notice Set operator permissions for confidential operations
     * @param operator The address to grant operator permissions to
     * @param expiration The expiration timestamp for the operator permissions
     */
 ...
 * @author RSIC-Zero Team
 * @notice Enhanced AMM with ZK proof-based verification for privacy-preserving token swaps
 * @dev This contract implements a fully homomorphic encryption (FHE) based automated market maker
 *      with zero-knowledge proof verification for secure and private token swaps and liquidity management.
 *      It combines FHEVM for encrypted computations with RISC Zero for cryptographic verification.
 *
 * Key Features:
 * - Privacy-preserving token swaps with encrypted reserves
 * - ZK proof verification for all operations
 * - Liquidity provision and removal with LP tokens
 * - Protocol fee management and distribution
 * - Replay protection and signature verification
 *
 * Security Model:
 * - All sensitive data (balances, reserves, amounts) are encrypted
 * - ZK proofs ensure calculation correctness without revealing data
 * - ECDSA signatures prevent unauthorized operations
 * - Nonce system prevents proof reuse attacks
 */
contract FHESwapSimpleGuarded is Ownable, SepoliaConfig {
    ...
    /// @notice Thrown when a proof has already been consumed
    error ProofAlreadyConsumed();

    /// @notice Thrown when the trusted verifier is not set
    error NoTrustedVerifier();
    ...
    /// @notice Thrown when reserves are not initialized
    error ReservesNotInitialized();
    ...
    /// @notice Thrown when caller is not authorized for fee operations
    error NotAuthorizedForFee();

    /// @notice Thrown when user has no LP balance
    error NoLPBalance();

    /// @notice Thrown when fee recipient is not set
    error FeeRecipientNotSet();

    /// @notice Thrown when only fee recipient can perform operation
    ...
    /// @notice Last calculated encrypted numerator for swap calculations
    euint64 private _lastNumerator;

    /// @notice Last calculated encrypted denominator for swap calculations
    euint64 private _lastDenominator;

    /// @notice Encrypted numerator for token0 in liquidity calculations
    euint64 private _liquidityNumerator0;

    /// @notice Encrypted numerator for token1 in liquidity calculations
    euint64 private _liquidityNumerator1;

    /// @notice Current encrypted reserve0 for liquidity calculations
    euint64 private _currentReserve0;

    /// @notice Current encrypted reserve1 for liquidity calculations
    euint64 private _currentReserve1;

    /// @notice Current encrypted total supply for liquidity calculations
    euint64 private _currentTotalSupply;

    /// @notice Flag indicating if this is the first liquidity addition
    bool private _isFirstAdd;

    /// @notice Address that receives protocol fees
    address public feeTo;

    /// @notice Flag indicating if protocol fees are enabled
    bool public feeEnabled;

    /// @notice Address authorized to set fee parameters
    address public feeToSetter;

    /// @notice Protocol fee in basis points (default: 5 bps = 0.05%)
    uint256 public protocolFeeBps = 5;

    /// @notice Last calculated K value (reserve0 * reserve1) for invariant tracking
    euint64 private _lastK;

    /// @notice Trusted verifier address for ZK proof signatures
    address public trustedVerifier;

    /// @notice Mapping to track consumed proof hashes to prevent replay attacks
    ...
    function addLiquidityWithProof(LiquidityParams calldata params) external {
        ...
        euint64 decryptedMinAmount0 = FHE.fromExternal(params.minAmount0, params.minAmount0Proof);
        euint64 decryptedMinAmount1 = FHE.fromExternal(params.minAmount1, params.minAmount1Proof);
        euint64 decryptedMinLiquidity = FHE.fromExternal(params.minLiquidity, params.minLiquidityProof);

        // 3. Set permissions for decrypted values
        FHE.allowThis(decryptedAmount0);
        FHE.allowThis(decryptedAmount1);
        FHE.allowThis(decryptedLiquidityMinted);
        FHE.allowThis(decryptedProtocolFeeLP);
        FHE.allowThis(decryptedMinAmount0);
        FHE.allowThis(decryptedMinAmount1);
        FHE.allowThis(decryptedMinLiquidity);
        ...
        // 6. Handle protocol fees if enabled
        ...
        // 8. Update K value for invariant tracking
        if (FHE.isInitialized(_reserve0) && FHE.isInitialized(_reserve1)) {
            _lastK = FHE.mul(_reserve0, _reserve1);
            FHE.allowThis(_lastK);
        }

        // 9. Set permissions for updated state
        FHE.allowThis(_reserve0);
        FHE.allowThis(_reserve1);
        ...
        if (_feeUpdated) {
            ...
            FHE.allow(_balances[feeTo], feeTo);
        }

        // 10. Mark proof as consumed and emit event
